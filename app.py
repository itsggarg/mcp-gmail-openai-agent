...
async def execute_agent_task(task):
    """
    Execute the agent task asynchronously with timeout protection.
    
    This function handles the complete lifecycle of an agent task:
    1. Build the agent and MCP server
    2. Connect to the MCP server
    3. Execute the task with 60-second timeout
    ...
    try:
        ...
        # Execute the task with 60-second timeout
        ...
        logger.error(f"Task execution timed out after {TASK_TIMEOUT} seconds")
        return {
            'success': False,
...
def execute_task():
    ...
    try:
        ...
        result = future.result(timeout=TASK_TIMEOUT + 5)  # Add 5 seconds buffer for cleanup
